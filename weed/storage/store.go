...
func (s *Store) DeleteVolumeNeedle(i needle.VolumeId, n *needle.Needle) (Size, error) {
	if v := s.findVolume(i); v != nil {
		...
		return v.deleteNeedle2(n, false)
	}
	return 0, fmt.Errorf("volume %d not found on %s:%d", i, s.Ip, s.Port)
}

// Добавяем новую функцию к хранилищу, кторая не только метит конент в инексе как уделённый, но и реально
// конент внтури dat файла нулями с последующим fsync. Нужно для реализации функционала
// "быстрого" удаления содердимого с диска. Например по соображениям безопсаности
// Данное удаление не возможно, только если на томе нет места.
