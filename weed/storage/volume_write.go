...
func (v *Volume) syncDelete(n *needle.Needle, erase bool) (Size, error) {
		...
		for {
			...
			for i := 0; i < len(currentRequests); i++ {
				if currentRequests[i].IsWriteRequest {
					...
					size, err := v.doDeleteRequest(currentRequests[i].N, currentRequests[i].DoEraseWhenDelete())
